...
EI <- function(t, Ts, Tf, EI0, EIsurg, EIfinal) #i pour l'individu auquel on s'interesse
{
  s = (EIfinal-EIsurg)/(Tf-Ts) #garde le regime pendant 150 jours (~5 mois) puis reprise progressive de nourriture jusqu'au 900eme jour (~3 ans)
  if (t<=0){
    EI0
  }
  else if (t<=Ts){
...
EqBW <- function(t, y, parameters){
  ...
  P3 <- parameters[7]
  P4 <- parameters[8]
  EE <- (parameters[2] + gf*y[1] + gl*y[2] + PA - parameters[1]*B + EI(t, P3, P4, parameters[1], P1, P2)*(B + partF*nf/pf + partL*nl/pl))/(1 + partF*nf/pf + partL*nl/pl)
  
  dF <- partF/pf * (EI(t, P3, P4, parameters[1], P1, P2) - EE)
  dL <- partL/pl * (EI(t, P3, P4, parameters[1], P1, P2) - EE)
  list(c(dF, dL))
}
...
for (i in 1:41){
  ...
  modelcost <- function(P) {
    sol <- lsoda(y=init, times=soltime, func = EqBW, parms = c(parameters,P[1],P[2],P[3],P[4]))
    return(modCost(sol,Data))
  }
  
  Fit <- modFit(f = modelcost, p = c(6000,9000,500,1000), lower=c(1000, 1000, ))
...
for (k in 1:41){
  ...
  parameters <- c(EI0[ind], K[ind], H[ind], age0[ind], EIsurg[ind], EIfinal[ind], Ts[ind], Tf[ind])
  ...
  S = c()
  for (i in 101:2301){
    graphEI[i] <- EI(graphtime[i], Ts[ind], Tf[ind], EI0[ind], EIsurg[ind], EIfinal[ind])
    graphEE[i] <- (K[ind] + gf*bestfit[i-100,2] + gl*bestfit[i-100,3] + ((1-Btef)*1.5-1)*4.184*(10*(bestfit[i-100,2]+bestfit[i-100,3])+625*H[ind]-5*(age0[ind]+(i-100)/365)-161) - EI0[ind]*(Bat+Btef) + EI(graphtime[i], Ts[ind], Tf[ind], EI0[ind], EIsurg[ind], EIfinal[ind])*(Btef + Bat + bestfit[i-100,2]/(C+bestfit[i-100,2])*nf/pf + C/(C+bestfit[i-100,2])*nl/pl))/(1 + bestfit[i-100,2]/(C+bestfit[i-100,2])*nf/pf + C/(C+bestfit[i-100,2])*nl/pl)
    if (graphEI[i] < 1.001*graphEE[i] & graphEI[i] >0.999*graphEE[i] & i >101) {S = c(S, i-100)}
  }
  
  plot(graphtime, graphEI, type="l", xlab="Days", ylab="Energy rate ", col=1, ylim=c(3000, 15000))
...
A_Fit <- modFit(f = A_modelcost, p = c(mean[],9000,500,1000))
